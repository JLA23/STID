...
public class ValiderModif implements ActionListener {
			...
			if (!commandes.getNumClient().getText().equals("") && commandes.getDonnees().exist("clients", "NumClient",
				...
				commandes.getBase().update("commandes",
						"numClient = " + commandes.getNumClient().getText() + ", CdeComClient = '"
								...
								+ commandes.getjHeureAtelier().getText().replaceAll(",", "\\.") + ", Corem = "
								+ commandes.getCheck().isSelected() + ", Prefabrication = "
	...
	private void validerTermes() {
		...
		if (termes.getNumFacture() == null) {
			exist = termes.getDonnees().exist("termes", "NumCommande, NumIndice, NumFacture",
					"NumCommande = " + termes.getNumeroCommande() + " AND NumIndice = " + termes.getNumeroIndice()
							+ " AND NumFacture is null");
		} else {
			exist = termes.getDonnees().exist("termes", "NumCommande, NumIndice, NumFacture",
					"NumCommande = " + termes.getNumeroCommande() + " AND NumIndice = " + termes.getNumeroIndice()
							+ " AND NumFacture = " + termes.getNumFacture());
		}
		if (exist) {
			if (termes.getDonnees().exist("commandes", "NumCommande", "NumCommande = " + termes.getNumeroCommande())) {
				if (termes.getNumFacture() == null) {
					termes.getBase().update("termes",
							...
							"numCommande = " + termes.getNumeroCommande() + " AND numIndice = "
					...
					termes.getBase().update("termes",
							"Lblterme = '" + apostrophe(termes.getjLibelle().getText()) + "', MntFour = "
									...
									+ termes.getjPrefabrication().getText().replaceAll(",", "\\."),
							"numCommande = " + termes.getNumeroCommande() + " AND numIndice = "
									+ termes.getNumeroIndice() + " AND numFacture = " + termes.getNumFacture());
				}
				String[] re = termes.getValeurDevises().get(termes.getDevises().getSelectedItem());
				termes.getBase().update("commandes", "CodeDevise = " + re[0],
				...
				JOptionPane.showMessageDialog(null, "Erreur : Numéro de Commande inconnu", "ATTENTION",
						JOptionPane.WARNING_MESSAGE);
			}
		} else {
			JOptionPane.showMessageDialog(null, "Numéro de Termes inexistant", "ATTENTION",
					JOptionPane.WARNING_MESSAGE);
		}
	}
	...
	public void validerFactures() {
		Factures factures = (Factures) classe;
		if (factures.getDonnees().exist("factures", "NumFacture",
			...
			if (!ModifieOuIdentique(valeursInit, valeursModifie)) {
				factures.getBase().update("factures",
						"MontantTaxe = " + valeursModifie[0] + " , valeur = " + factures.getjValeur().getText()
								...
								+ "', AnneeValeur = " + factures.getjAnneeValeur().getText() + ", CodeDevise = "
								+ re[0],
						"numFacture = " + factures.getjNumFacture().getText());
			...
			JOptionPane.showMessageDialog(null, "Numéro de Facture inexistant", "ATTENTION",
					JOptionPane.WARNING_MESSAGE);
		}
	}
