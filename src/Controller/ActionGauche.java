...
public class ActionGauche implements ActionListener {
	...
	public ActionGauche(Devis d, String classe, String fonction) throws ParseException {
		this.devis = d;
		this.classe = classe;
		this.fonction = fonction;
	}

	public ActionGauche(Client d, String classe, String fonction) throws ParseException {
		this.client = d;
		this.classe = classe;
		this.fonction = fonction;
	}

	public ActionGauche(Commandes c, String classe, String fonction) throws ParseException {
		this.commandes = c;
		this.classe = classe;
		this.fonction = fonction;
	}

	public ActionGauche(Termes t, String classe, String fonction) throws ParseException {
		this.termes = t;
		this.classe = classe;
		this.fonction = fonction;
	}

	public ActionGauche(Factures f, String classe, String fonction) throws ParseException {
		this.factures = f;
		this.classe = classe;
		this.fonction = fonction;
	}

	public ActionGauche(Salarie s, String classe, String fonction) throws ParseException {
		this.salarie = s;
		this.classe = classe;
		this.fonction = fonction;
	}
	...
	private void actionDevis() throws ParseException {
		if (devis.getDonnees().exist("devis", "NumDevis", "numDevis = " + devis.getjNumDevis().getText())) {
			...
			String[] res = devis.getDonnees().fiche("*, c.nomclient", "devis as d, clients as c",
					"d.numclient = c.numclient and d.numDevis = " + numero);
			devis.initModif(res);
	...
	private void actionCommandes() throws ParseException {
		if (commandes.getDonnees().exist("commandes", "Numcommande",
			...
			if (!commandes.getNumClient().getText().equals("") && commandes.getDonnees().exist("clients", "NumClient",
				...
				String[] res = commandes.getDonnees().fiche("*, c.nomclient", "commandes as d, clients as c",
		...
		if (client.getDonnees().exist("clients", "Numclient", "numclient = " + client.getjNumClient().getText())) {
			...
			if (!ModifieOuIdentique(valeursInit, valeursModifie)) {
				client.getBase().update("clients",
						"Nomclient = '" + client.getjName().getText() + "', Adresse1 = '" + apostrophe(valeursModifie[2])
			...
			while (!client.getDonnees().exist("clients", "numClient", "numClient = " + numero)) {
				numero -= 1;
			}
